            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{ active: isOne }"  @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{ 'icon-UP': isAsc, 'icon-DOWN': isDesc }"
                    ></span
                  ></a>
                </li>
                <li :class="{ active: isTwo }" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{ 'icon-UP': isAsc, 'icon-DOWN': isDesc }"
                    ></span
                  ></a>
                </li>
              </ul>
...
export default {
  ...
  computed: {
    // ...mapState({
    //   // searchList:(state) => state.search.searchList.goodsList
    // })
  ...
  //组件挂在完毕 执行一次【仅仅一次】
  mounted() {
    //发请求前给服务器参数【searchParams参数发生变化数值带给服务器】
    this.getData();
  },
  methods: {
    ...
    moveCateName() {
      //将带给服务器的参数置空、还需要向服务器发请求
      //优化空参数，设置对应字段undefined。当前字段不会带给服务器。
      this.searchParams.categoryName = undefined;
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      //地址栏修改，进行路由跳转(现在的路由只跳转到自己这里)
      //严谨：本意是删除query，如果路径中出现params不应该删除，路由跳转应该带着
    ...
    trademarkInfo(trademark) {
      ...
      //再次发请求获取search模块进行展示
      // this.getData(attr,attrValue);
      this.getData()
    },
    //自定义回调，收集平台属性回调函数
    attrInfo(attr,attrValue){
      //整理售卖属性的参数 [数组] ["属性ID:属性值:属性名"]
  ...
  watch: {
    //监听路由信息是否发生变化，如果发生变化，再次发期请求
    $route(newValue, oldValue) {
      //再次发请求之前整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      //再次发期ajax请求，
      this.getData();
      //每次请求完毕，把响应1、2、3级分类的id指控，让他接受下一次的响应1、2、3id
      //分类名字与关键字不用清理；因每吃一次路由发生变化时，都会给它赋予新的数据
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
