        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(yanse, index) in spuSaleAttrList" :key="yanse.id">
                <dt class="title">{{ yanse.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: yanse1.isChecked == 1 }"
                  v-for="(yanse1, index) in yanse.spuSaleAttrValueList"
                  :key="yanse1.id"
                  @click="changeActive(yanse1,yanse.spuSaleAttrValueList)"
                >
                  {{ yanse1.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
 ...
 //1:在点击加入购物车这个按钮的时候，做的第一件事情，将参数带给服务器（发请求），通知服务器加入购车的产品是谁
  ...
  try {
    ...
    await  this.$store.dispatch('addOrUpdateShopCart',{
      ...
      //3路由跳转
      //4 路由跳转时，需要将产品信息带给下一级路由组件，
      //一些简单的数据skuNum。通过query形式给路由组件传递过去
      //产品信息数据【复杂:skuInfo】通过会话存储（不持久化，会话结束数据消失
      //本次存储||会话存储，一般存储的是字符串
      sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo)) //转化为字符串
